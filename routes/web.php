...
/*
|--------------------------------------------------------------------------
| Admin Routes (Rank 7+ required)
|--------------------------------------------------------------------------
|
| Administrative interface for managing users, content and system settings.
| Protected by admin middleware requiring rank 7 or higher.
|
*/

# Admin routes with admin middleware
$app->group(['prefix' => $path . 'admin', 'middleware' => ['auth', 'admin']], function () use ($app) {
    ...
    $app->get('users/{id}', 'AdminController@userDetails');
    $app->put('users/{id}', 'AdminController@updateUser');
    $app->post('users/{id}/ban', 'AdminController@banUser');
    $app->post('users/{id}/unban', 'AdminController@unbanUser');
    
    # Articles management
    $app->get('articles', 'AdminController@articles');
    $app->post('articles', 'AdminController@createArticle');
    $app->delete('articles/{id}', 'AdminController@deleteArticle');
    
    # System settings
    $app->get('settings', 'AdminController@settings');
    
    # Admin logs
    $app->get('logs', 'AdminController@logs');
});
